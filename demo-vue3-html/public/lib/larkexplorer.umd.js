(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function PosFromDegreeArray(positions) {
        ...
        var points = Cesium.Cartesian3.fromDegreesArray(result);
        return points;
    }
    /**
     * WGS84坐标转笛卡尔坐标
    ...
    function getScale(viewer) {
        ...
        do {
            leftPos = viewer.camera.pickEllipsoid(new Cesium.Cartesian2(midX, midY));
            rightPos = viewer.camera.pickEllipsoid(new Cesium.Cartesian2(midX + 2, midY));
            if (!leftPos || !rightPos) {
                midX -= 1;
            }
        } while (!leftPos || !rightPos);
        var lg = Cesium.Cartographic.fromCartesian(leftPos);
        var rg = Cesium.Cartographic.fromCartesian(rightPos);
        var lon1 = Cesium.Math.toDegrees(lg.longitude);
        var lat1 = Cesium.Math.toDegrees(lg.latitude);
        var lon2 = Cesium.Math.toDegrees(rg.longitude);
        var lat2 = Cesium.Math.toDegrees(rg.latitude);
    ...
    var viewExtend = function (viewer) {
        ...
        if (typeof extend === "undefined") {
            ...
            var minx = Cesium.Math.toDegrees(upperLeftCartographic.longitude); //弧度转经纬度
            var maxx = Cesium.Math.toDegrees(lowerRightCartographic.longitude); //弧度转经纬度
            var miny = Cesium.Math.toDegrees(lowerRightCartographic.latitude); //弧度转经纬度
            var maxy = Cesium.Math.toDegrees(upperLeftCartographic.latitude); //弧度转经纬度
            params.minx = minx;
            params.maxx = maxx;
            params.miny = miny;
            params.maxy = maxy;
    ...
    var flyToExtent = function (viewer, extent, time, callback) {
        ...
        var rectangle = new Cesium.Rectangle(ex[0], ex[1], ex[2], ex[3]);
        viewer.camera.flyTo({
            ...
            appearance: new Cesium.EllipsoidSurfaceAppearance({
                ...
                material: new Cesium.Material({
                    fabric: {
                        type: "Water",
                        uniforms: {
                            blendColor: new Cesium.Color(1, 0, 0, 0.3),
                            normalMap: img,
                            frequency: 200,
                            animationSpeed: 0.01,
                            amplitude: 10,
    ...
    function addTextureUniform(options) {
        ...
        var ws = wrapS || Cesium.TextureWrap.REPEAT;
        var wt = wrapT || Cesium.TextureWrap.REPEAT;
        var miniF = options.minificationFilter || Cesium.TextureMinificationFilter.LINEAR;
        var magF = options.magnificationFilter || Cesium.TextureMagnificationFilter.LINEAR;
        var source = new Image();
        source.src = imgSrc;
        source.addEventListener("load", function () {
            var sampler = new Cesium.Sampler({ ws: ws, wt: wt, miniF: miniF, magF: magF });
            var texture = new Cesium.Texture({ context: context, source: source, sampler: sampler });
    ...
    function render(scene, passStateFramebuffer) {
        ...
        var backgroundColor = defaultValue(scene.backgroundColor, Cesium.Color.BLUE);
        if (scene._hdr) {
        ...
        Cesium.defined(globe) && globe.beginFrame(frameState);
    ...
    var WaterPrimitive = /** @class */ (function () {
        ...
        function WaterPrimitive(viewer, options) {
            ...
            positions.forEach(function (p) {
                ...
                _this._positions.push(Cesium.Cartesian3.fromRadians(longitude, latitude, _this._height));
            });
            _x /= total;
            _y /= total;
            _z /= total;
            ...
            this._rwPos = Cesium.Cartesian3.fromRadians(cX, cY, this._height);
            this._originRWPos = this._rwPos.clone();
            this._normal = Cesium.Ellipsoid.WGS84.geodeticSurfaceNormal(this._rwPos);
            this._waterPlane = Cesium.Plane.fromPointNormal(this._rwPos, this._normal);
            var _a = this._waterPlane, normal = _a.normal, distance = _a.distance;
            var x = normal.x, y = normal.y, z = normal.z;
            this._reflectMatrix = new Cesium.Matrix4(-2 * x * x + 1, -2 * x * y, -2 * x * z, -2 * x * distance, -2 * y * x, -2 * y * y + 1, -2 * y * z, -2 * y * distance, -2 * z * x, -2 * z * y, -2 * z * z + 1, -2 * z * distance, 0, 0, 0, 1);
            ...
            this._initUniforms = {
                ...
                waterColor: defaultValue(options.waterColor, Cesium.Color.fromCssColorString("#00aeff")),
                waterAlpha: defaultValue(options.waterAlpha, 0.9),
                rf0: defaultValue(options.reflectivity, 0.3),
                lightDirection: defaultValue(options.lightDirection, new Cesium.Cartesian3(0, 0, 1)),
            ...
            Cesium.UniformState.prototype.updateFrustum = function (frustum) {
                Cesium.Matrix4.clone(defaultValue(frustum.customProjectionMatrix, frustum.projectionMatrix), this._projection);
        ...
        Object.defineProperty(WaterPrimitive.prototype, "height", {
            ...
            set: function (value) {
                this._height = value;
                var rwpCa = Cesium.Cartographic.fromCartesian(this._originRWPos);
                var newRwpCa = Cesium.Cartesian3.fromRadians(rwpCa.longitude, rwpCa.latitude, this._height);
                var move = Cesium.Cartesian3.subtract(newRwpCa, this._originRWPos, new Cesium.Cartesian3());
        ...
        Object.defineProperty(WaterPrimitive.prototype, "waterColor", {
            ...
            set: function (value) {
                this._material.uniforms.waterColor = Cesium.Color.fromCssColorString(value);
            },
            enumerable: false,
            configurable: true
        });
        ...
        WaterPrimitive.prototype._createReflectionWaterMaterial = function () {
            ...
            var initUniforms = {
                ...
                image: texture,
                time: 0,
                fixedFrameToEnu: Cesium.Matrix4.toArray(this._getFixedFrameToEastNorthUpTransformFromWorldMatrix()),
            };
        ...
        WaterPrimitive.prototype._updateVirtualCamera = function (camera) {
            ...
            var reflectorPlane = Cesium.Plane.fromPointNormal(this._rwPos, this._normal);
        ...
        WaterPrimitive.prototype.preRender = function (scene) {
            ...
            this._material.uniforms.fixedFrameToEnu = Cesium.Matrix4.toArray(this._getFixedFrameToEastNorthUpTransformFromWorldMatrix());
        ...
        WaterPrimitive.prototype._getFixedFrameToEastNorthUpTransformFromWorldMatrix = function () {
            var EnuToFixedFrame = Cesium.Transforms.eastNorthUpToFixedFrame(this._rwPos);
            var fixedFrameToEnu = Cesium.Matrix4.inverse(EnuToFixedFrame, new Cesium.Matrix4());
    ...
    var AroundPoint = /** @class */ (function () {
        ...
        AroundPoint.prototype._rotate = function () {
            var delTime = Cesium.JulianDate.secondsDifference(this.viewer.clock.currentTime, this._startTime);
            var angle = 360 / this._time;
            var heading = Cesium.Math.toRadians(delTime * angle);
